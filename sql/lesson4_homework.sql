...
/*取出first_name第1個字元是A,同時first_name的字元長度是3到5的資料*/
	SELECT * FROM customer where  substring(first_name,1,1)='A'  and length(first_name) between 3 and 5  ;
/*取出first_name,前3字為Bra,但last_name不是Motley*/
	SELECT * FROM customer where  substring(first_name,1,3)='Bra'  andlast_name !='Motley'  ;
